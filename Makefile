...
CFLAGS = -Wall $(LIB) -c -fno-builtin -W -Wstrict-prototypes \
         -Wmissing-prototypes -g -m32 -fno-stack-protector
...
${BUILD_DIR}/syscall_user.o: userprog/syscall_user.c kernel/syscall.h userprog/syscall_user.h lib/stdint.h
	${CC} ${CFLAGS} $< -o $@


${BUILD_DIR}/syscall.o : kernel/syscall.c lib/stdint.h\
 thread/thread.h kernel/syscall.h lib/stdio.h device/console.h lib/string.h
	${CC} ${CFLAGS} $< -o $@


${BUILD_DIR}/stdio.o : lib/stdio.c lib/stdio.h kernel/interrupt.h \
kernel/global.h lib/string.h kernel/syscall.h  lib/kernel/print.h
	${CC} ${CFLAGS} $< -o $@
...
hd:
	...
	dd if=/dev/zero of=/myos/myos.img count=10000
	dd if=${BUILD_DIR}/mbr.bin of=/myos/myos.img bs=512 count=1 conv=notrunc
	dd if=${BUILD_DIR}/loader.bin of=/myos/myos.img bs=512 count=4 seek=2 conv=notrunc
	dd if=${BUILD_DIR}/kernel.bin of=/myos/myos.img bs=512 count=200 seek=9 conv=notrunc 
